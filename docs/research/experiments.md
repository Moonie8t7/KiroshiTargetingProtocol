...
Three mechanisms were tested in game. Two work and carry the mod; one does not and is not used.
These results decide what KSTP can claim.
...
| File | Contents |
|---|---|
| `docs/evidence/2026-08-16-lab-log.txt` | Timestamped stat before/after values and HUD readouts |
...
come from the logged stat values and the HUD state transitions, both of which are in the log.

---

## 1. Target-side lock-time inflation: works

**Tested.** Whether a `SmartGunTimeToLock*ComponentMultiplier` modifier applied to a target NPC's
`StatsObjectID` reaches the native smart-gun handler.

**Result.** It does. Applying `SmartGunTimeToLockHeadComponentMultiplier` at x1000 to a single NPC
held that NPC at `Locking` indefinitely while unmodified NPCs in the same encounter locked
normally.
...
- The faction and threat-class axes are enforceable per candidate, in pure redscript. No RED4ext
  plugin is required.
- The mechanism is preventive only. It stops a lock from completing and cannot release a lock that
  has already completed, so enforcement must reach a candidate before its lock timer finishes.
  Enforcement driven off the delayed blackboard listener (`smart-gun-internals.md` section 5) is
  reactive and can lose that race; a lock that completes first stays.
...
- **Disabling every class does not disable targeting.** With Head and Chest both zeroed the lock
  ...
  `cyberpunk\items\weapon.script:1533`. A protocol that leaves a target with no enabled class does
  not refuse that target; it engages the head. "Do not engage this target" belongs on the
  target-side mechanism in section 1, not on the body-part axis.

### Supporting confirmations from the same runs

- The cyberware delivery path fires. Head and WeakSpot read `2.00` on the held weapon where vanilla
  ships `0`, sourced from `KSTP.Mod_TrackOn_*`, while Chest `3`, Leg `2` and Mechanical `1` match
  the vanilla baseline. `Prereqs.SmartWeaponHeldPrereq` resolves despite being absent from the
  script dump, and the `ApplyStatGroupEffector` path applies the group.
- The cyberware costs zero capacity, achieved with an empty `statModifiers` list rather than a
  `Variants.Humanity0Cost` reference.
- Teardown is reliable. Every removal pass logged `0 failure(s)` across dozens of applications.
...
`SmartGunTimeToLock*ComponentMultiplier` is not uniformly initialized across the family.

| Class | Value on an NPC in vanilla |
|---|---|
| Head | `1.0` |
| Chest | approximately `1e21` |

Multiplying an already-enormous value repeatedly saturates it, and a saturated stat produces no
further change in behavior:

```
1e21 -> 1e24 -> 1e27 -> 1e30 -> 1e33 -> 1e36 -> 340282346638528859811704183484516925440 (FLT_MAX)
```

A test written against the Chest multiplier therefore reports a working mechanism as dead. Two
rules follow for any stat probe in this codebase:
